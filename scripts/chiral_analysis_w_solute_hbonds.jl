using CliffordNumbers # for computing the chirality measure
using Chemfiles # for reading in the trajectory
using LinearAlgebra # for computing the norm of vectors
using DelimitedFiles # for writing out data
using BenchmarkTools # for timing code
...
# clifford algebra basis vectors e1, e2, e3
...
#output = "/home/dsuvlu/git/enantiomers/results/2-butanol/r/"
#traj_name = "trajectory_10ns.dcd"
#traj_name = "r_trajectory_10ns.dcd"
#pdb_name = "water.pdb"
#pdb_name = "r_2-butanol_water.pdb"
...
oxy = Int.(Chemfiles.evaluate(sel_oxy, frame))
n_o = length(oxy)

# initialize dictionaries
nhbs, donors, chains, trivecs, min_dist = initialize_dictionaries(oxy)

# select hydrogen bonding solute atoms, initialize solute dictionaries
if solute_flag
    sel_sol = Selection("(type O or type N) and not resname HOH")
    solute = Int.(Chemfiles.evaluate(sel_sol, frame))
    sol_dist = Dict{Int, Float64}()
    sol_hbonds = Dict{Int, Vector{HBond}}()
    for i in solute
        sol_dist[i] = 0.0
        sol_hbonds[i] = Vector{HBond}()
        println(name(Atom(frame, i)))
    end
    # get information about solute donors and acceptors
    sol_info = solute_donors_acceptors(frame, solute)
end
...
for f in 0:n_frames-1
    ...
    if solute_flag
        ...
        # find minimum distance between solute and water oxygens
        minimum_distance!(min_dist, pos_static, ucell, oxy, solute, sol_dist)
        # find solute hbonds
        find_solute_hbonds!(sol_hbonds, sol_info, min_dist, params)
    end
    ...
    if solute_flag
        ...
        write_solute_hbonds!(sol_hbonds, f, output)
    end
